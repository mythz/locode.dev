...
export default ({ question }: Props) => {
    const pvqUrl = `https://pvq.app/questions/${question.id}/${question.post.slug}`
    const now = new Date()
    return (
        <div className="mb-20" data-module="mjs/question.mjs">
            <article id="question" data-postid={question.id} data-createdby={question.post.createdBy}>
                <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 lg:text-4xl xl:text-5xl">
                    <span className="block xl:inline">{question.post.title}</span>
                </h1>
                <div className="my-4 py-2 flex justify-between border-b border-gray-200 dark:border-gray-700">
                    <div className="flex gap-x-6 text-sm text-gray-500 dark:text-gray-400">
                        <div>
                            ...
                            <b className="ml-1">{relativeTimeFromDate(question?.post.creationDate)}</b>
                        </div>
                        {question?.post.lastEditDate != null && question?.post.lastEditDate !== question?.post.creationDate
                            ? (<div>
                                <span>last updated</span>
                                <b className="ml-1">{relativeTimeFromDate(question?.post.lastEditDate)}</b>
                            </div>)
                            : null}
                ...
                <div id="@Question.Post.Id" className="flex">
                    <div className="md:w-32 pr-2">
                        ...
                        {question?.post.createdBy
                            ? (<div className="my-4 flex flex-col items-center text-sm">
                                <img className="w-10 h-10 xl:w-16 xl:h-16 bg-cover inline-block" src={getAvatarUrl(question.post.createdBy)} />
                                <div className="mt-1 hidden md:block text-center whitespace-nowrap font-semibold">{question.post.createdBy}</div>
                                <div className="mt-1 flex items-center">
                                    <svg className="h-4 w-4 text-yellow-500" viewBox="0 0 32 32">
                                        <circle cx="16" cy="16" r="8" fill="currentColor" />
                                    </svg>
                                    <span className="font-semibold" data-rep-user={question.post.createdBy}>1</span>
                                </div>
                            </div>)
                            : null}
                    ...
                    <div>
                        <div id={`preview-${question.id}`} className="pb-6 preview xl:flex-grow prose">
                            {renderMarkdown(question.post.body)}
                        </div>
                        ...
                        <div className="question-footer flex justify-between w-full items-center">
                            ...
                            <div className="ml-2 text-xs">
                                <div className="flex">
                                    <span>{!question.post.lastEditDate ? "created" : "edited"}</span>
                                    <dd className="ml-1 text-gray-600 dark:text-gray-300">
                                        <time datetime={getModifiedDate(question?.post.lastEditDate)}>{formatDate(getModifiedDate(question.post))}</time>
                                        {question.post.modifiedBy && question.post.modifiedBy !== question.post.createdBy
                                            ? (<span>
                                                <span> by </span>
                                    ...
                                    <div className="flex w-full">
                                        ...
                                        <div className="ml-3">
                                            <p className="text-sm text-red-700">
                                                ...
                                                <span className="ml-1 font-semibold">{question.post.lockedReason}</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>)
